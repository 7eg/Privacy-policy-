

---

# Privacy Policy / سياسة الخصوصية

**Effective Date / تاريخ السريان:** 30/04/2026

---

# English Version

## Privacy Policy

Thank you for using **Our Apps**.

This Privacy Policy explains how we collect, use, and protect your information when you use our mobile application.

### 1. Information We Collect

Depending on your app functionality, we may collect:

- Basic account information
- Device information
- App usage data
- Internet connection information
- Login credentials (if applicable)
- Uploaded content or files (if applicable)

### 2. How We Use Information

We use collected information to:

- Provide and improve app functionality
- Maintain security and prevent misuse
- Improve user experience
- Provide customer support
- Fix technical issues

### 3. Data Sharing

We do not sell or rent your personal information.

We may share information only in the following situations:

- To comply with legal requirements
- To protect app security and prevent fraud
- With trusted service providers that help operate the app

### 4. Data Storage and Security

We take reasonable measures to protect your information from unauthorized access, loss, misuse, or disclosure.

### 5. Third-Party Services

Our app may use third-party services such as:

- Google Play Services
- Firebase
- Analytics tools
- Authentication services

These services may collect information according to their own privacy policies.

### 6. Children's Privacy

Our app is not intended for children under 13 years old.

We do not knowingly collect personal information from children.

### 7. Your Rights

You may request to:

- Access your data
- Correct inaccurate information
- Request deletion of your data

### 8. Changes to This Policy

We may update this Privacy Policy from time to time.

Any changes will be posted on this page.

### 9. Contact Us

If you have questions about this Privacy Policy, contact us:

**Email:** toxpoisonic@gmail.com

---

# النسخة العربية

## سياسة الخصوصية

شكرًا لاستخدامك تطبيق **[اسم التطبيق]**.

توضح سياسة الخصوصية هذه كيفية جمع معلوماتك واستخدامها وحمايتها عند استخدام تطبيق الهاتف المحمول الخاص بنا.

### 1. المعلومات التي نقوم بجمعها

اعتمادًا على وظائف التطبيق، قد نقوم بجمع:

- معلومات الحساب الأساسية
- معلومات الجهاز
- بيانات استخدام التطبيق
- معلومات الاتصال بالإنترنت
- بيانات تسجيل الدخول (إن وجدت)
- الملفات أو المحتوى الذي يتم رفعه (إن وجد)

### 2. كيفية استخدام المعلومات

نستخدم المعلومات التي يتم جمعها من أجل:

- تقديم وتحسين وظائف التطبيق
- الحفاظ على الأمان ومنع سوء الاستخدام
- تحسين تجربة المستخدم
- تقديم الدعم الفني
- إصلاح المشاكل التقنية

### 3. مشاركة البيانات

نحن لا نقوم ببيع أو تأجير معلوماتك الشخصية.

قد نشارك البيانات فقط في الحالات التالية:

- الامتثال للمتطلبات القانونية
- حماية أمان التطبيق ومنع الاحتيال
- مع مزودي الخدمات الموثوقين الذين يساعدون في تشغيل التطبيق

### 4. تخزين البيانات والأمان

نتخذ إجراءات معقولة لحماية معلوماتك من الوصول غير المصرح به أو الفقدان أو سوء الاستخدام أو الإفصاح.

### 5. خدمات الطرف الثالث

قد يستخدم التطبيق خدمات طرف ثالث مثل:
Google Play Services
- Firebase
- أدوات التحليل
- خدمات تسجيل الدخول

قد تقوم هذه الخدمات بجمع معلومات وفقًا لسياسات الخصوصية الخاصة بها.

### 6. خصوصية الأطفال

هذا التطبيق غير مخصص للأطفال دون سن 13 عامًا.

نحن لا نجمع معلومات شخصية من الأطفال عن قصد.

### 7. حقوق المستخدم

يمكنك طلب:

- الوصول إلى معلوماتك
- تصحيح المعلومات غير الدقيقة
- حذف معلوماتك

### 8. التعديلات على سياسة الخصوصية

قد نقوم بتحديث سياسة الخصوصية من وقت لآخر.

سيتم نشر أي تغييرات على هذه الصفحة.

### 9. التواصل معنا

إذا كانت لديك أي أسئلة حول سياسة الخصوصية، يمكنك التواصل معنا عبر:

**البريد الإلكتروني: toxpoisonic@gmail.com

---



